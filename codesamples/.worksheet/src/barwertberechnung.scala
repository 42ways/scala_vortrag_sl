...
object barwertberechnung {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(104); 
  ...
  val vp = new Person("M", 175, 68, DateFactory(1990, 1, 1));System.out.println("""vp  : <error> = """ + $show(vp ));$skip(139); 

  val barwertBerechnungKlassisch =
    barwert(InterrestRate.constant0175.rate, klassisches_qx("2009/2011", vp.sex), Spektrum.yearly.fn)_;System.out.println("""barwertBerechnungKlassisch  : <error> = """ + $show(barwertBerechnungKlassisch ));$skip(104); 
  ...
  val barwertBerechnungUnisex =
    barwert(InterrestRate.constant0175.rate, unisex_qx("2009/2011"), Spektrum.yearly.fn)_;System.out.println("""barwertBerechnungUnisex  : <error> = """ + $show(barwertBerechnungUnisex ));$skip(98); 
  println("Barwert unisex  = " + barwertBerechnungUnisex(0, 12, vp.age(DateFactory(2014, 1, 1))));$skip(149); 

  val barwertBerechnungBmi =
    barwert(InterrestRate.constant0175.rate, bmi_qx("2009/2011", vp.sex, vp.groesse, vp.gewicht), Spektrum.yearly.fn)_;System.out.println("""barwertBerechnungBmi  : <error> = """ + $show(barwertBerechnungBmi ));$skip(92); 
  println("Barwert bmi  = " + barwertBerechnungBmi(0, 12, vp.age(DateFactory(2014, 1, 1))));$skip(19); 
